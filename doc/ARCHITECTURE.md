...
## 3. 三层架构：Skill → DAG Engine → Subagents

```
┌─────────────────────────────────────────┐
│  Skill 层 (SKILL.md)                     │
│  定义：可用子Agent、依赖关系、并行组      │
│  位置：.agent/skills/<name>/SKILL.md     │
│  产出：DAG 结构定义                      │
└──────────────────┬──────────────────────┘
                   │
┌──────────────────▼──────────────────────┐
│  Planner (Lead Agent)                    │
│  职责：需求澄清、风格建议、结果解读       │
│  不负责：执行调度（DAG Engine 负责）      │
│  通信：chat() 开放式对话                 │
└──────────────────┬──────────────────────┘
                   │
┌──────────────────▼──────────────────────┐
│  DAG Engine (唯一执行层)                 │
│  拓扑排序 → 依赖解析 → 并行 fan-out       │
│  状态流转 → 错误隔离 → 结果汇总          │
└──────────────────┬──────────────────────┘
                   │
     ┌─────────────┼─────────────┐
...
```
product_analysis → script → model_images ∥ scene_images → clips → final_video
                              ↑ 并行组 image_generation ↑
```
...
| | 旧（v1） | 新（v2） |
|---|---------|---------|
| 角色 | 执行主编，调度所有子Agent | 规划者，需求澄清+结果解读 |
| 执行 | 通过 task_tool 调度 | DAG Engine 确定性执行 |
| 入口 | generate_video() | chat() |
